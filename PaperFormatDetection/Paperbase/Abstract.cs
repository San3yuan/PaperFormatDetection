...
namespace PaperFormatDetection.Paperbase
{
    ...
    { /// 标题9个参数 顺序分别是 0该部分是否必要 1标题之间空格数目 2对齐方式 3行间距 4段前间距 5段后间距 6字体 7字号 8是否加粗
        ...
        protected string[] abstext = { "2", "0", "0", "0", "黑体", "0", "0", "0" };
        ...
        public void detectabstitle(List<Paragraph> list, WordprocessingDocument doc)
        {
            ...
            if (list.Count <= 0)
            {
                if (bool.Parse(abstitle[0]))
                    Util.printError("论文缺少中文摘要部分");
            }
            else
            {/// 标题9个参数 顺序分别是 0该部分是否必要 1标题之间空格数目 2对齐方式 3行间距 4段前间距 5段后间距 6字体 7字号 8是否加粗
                string temp = Tool.getFullText(list[0]).Trim(); //list[0].InnerText.Trim()
                if (temp.Length != 4) Util.printError("“中文摘要”标题之间有空格");
                if (!Util.correctJustification(list[0], doc, abstitle[2])) Util.printError("“摘要”标题对齐方式错误，应为居中");
                if (!Util.correctSpacingBetweenLines_line(list[0], doc, abstitle[3])) Util.printError("“摘要”标题行间距错误，应为1倍行距");
            ...
            {/// 关键词的参数 顺序是 0字体  1 字号 3 最小 4最大
                ...
                {
                    if (change == 0) { if (!Util.correctIndentation(list[i], doc, abstext[7])) Util.printError("中文关键词不需要首行缩进"); }
                    exist = 1;
                    ...
                    foreach (Run run in runlist)
                    {
                        ...
                        {
                            if (change == 1) { if (ky.Replace(" ", "") != "；" && ky.Replace(" ", "") != "：" && !Util.correctRunFonts(run, list[i], doc, "仿宋_GB2312", "Times New Roman")) { Util.printError("中文关键词的内容字体错误，中文应为仿宋_GB2312，英文应为Times New Roman" + "           " + ky); } }
                            if (change == 0) if (ky.Replace(" ", "") != "；" && ky.Replace(" ", "") != "：" && !Util.correctRunFonts(run, list[i], doc, "宋体", "Times New Roman")) { Util.printError("中文关键词的内容字体错误，应为黑体，英文应为Times New Roman" + "       " + ky); }
                        }
                    }
        ...
        public void detectabstitleE(List<Paragraph> list, WordprocessingDocument doc)
        {
            ...
            else
            {
                ...
                if (!Util.correctSpacingBetweenLines_line(list[0], doc, abstitleE[3])) Util.printError("“Abstract”标题行间距错误，应为单倍行距");
                ...
                if (change == 0) { if (!Util.correctfonts(list[0], doc, "黑体", abstextE[4])) Util.printError("“Abstract”标题字体错误，应为Times New Roman"); }
            ...
            {/// 正文7个参数 顺序分别是 0缩进  1行间距 2段前间距 3段后间距 4字体 5字号
                ...
                string fulltext = Tool.getFullText(list[i]);
                string str = fulltext.Replace(" ", "").ToLower();
                temp = (temp.Length > 10 ? temp.Substring(0, 10) : temp);
                if (temp.Length == 0) continue;
                if (str.Length > 9 && str.Substring(0, 8) == "keywords") break;
                if (!Util.correctIndentation(list[i], doc, abstext[0])) Util.printError("abstract正文段落缩进错误，应为每段落首行缩进2字符" + "  " + temp);
                if (!Util.correctSpacingBetweenLines_line(list[i], doc, abstextE[1])) Util.printError("abstract正文行距错误，应为20磅" + "  " + temp);
            ...
            for (int n = 1; n < list.Count; n++)
            {
                ...
                string str = fulltext.Replace(" ", "").ToLower();
                if (fulltext.Length > 10 && fulltext1.Replace(" ", "").Length == 0 && str.Substring(0, 8) == "keywords") { exist = 1; Util.printError("abstract正文应与keywords:空一行"); }
                if (str.Length > 9 && str.Substring(0, 8) == "keywords")
                {
                    exist = 1;
                    int daxie = Util.getFullText(list[n]).Trim().Length;
                    int rule = 0; int ay = 0;
                    int iu = 1; int ko = 0;
                    for (; ay < daxie - 2; ay++)
                    {
                        if (fulltext1[ay] == ' ' && fulltext1[ay] != ' ' && !(fulltext1[ay + 1] <= 'Z' && fulltext1[ay + 1] >= 'A')) Util.printError("英文关键词中的每个单词的首字母都应该大写" + fulltext1[ay] + fulltext1[ay + 1] + fulltext1[ay + 2]);
                        if (change == 0) { if (fulltext1[ay] == '；' && fulltext1[ay + 1] == ' ') Util.printError("英文关键词的中文分号后不需要空格"); }
                        if (change == 1) { if (fulltext1[ay] == '；' && fulltext1[ay + 1] != ' ') Util.printError("英文关键词的中文分号后需要一个空格"); }
                    }
                    if (change == 0) { if (!Util.correctIndentation(list[n], doc, abstext[7])) Util.printError("英文关键词“Key Words:”不需要首行缩进"); }
                    if (!(fulltext.Length > 10 && fulltext.Replace(" ", "").Substring(0, 9) == "KEYWORDS：")) { exist = 1; Util.printError("“KeyWords：”格式错误"); }
                    if (fulltext.Length > 10 && fulltext.Replace(" ", "").Substring(0, 8) != "KEYWORDS") { exist = 1; Util.printError("请注意“KeyWords：”中的大小写"); }
                    if (fulltext.Length > 10 && (fulltext.Substring(0, 10) != "Key Words：" && fulltext.Substring(0, 10) != "Key Words:") && (fulltext.Replace(" ", "").Substring(0, 10) == "Key Words：" || fulltext.Replace(" ", "").Substring(0, 10) == "Key Words:") && (fulltext.Trim().Substring(0, 9) != "KeyWords：" && fulltext.Trim().Substring(0, 9) != "KeyWords:")) { exist = 1; Util.printError("“KeyWords：”中间缺少空格"); }
                    if (fulltext.Length > 10 && (fulltext.Trim().Substring(0, 10) == "Key Words：" || fulltext.Trim().Substring(0, 10) == "Key Words:")) { exist = 1; Util.printError("“KeyWords：”中间多出不必要的空格"); }
                    ...
                    if (fulltext.Replace(" ", "").Substring(0, 9) == "KeyWords：")
                    {
                        ...
                        foreach (Run run in runlist)
                        {
                            ...
                            {
                                if (change == 0) { if (ko == 0) { if (!Util.correctRunFonts(run, list[n], doc, "黑体", abstextE[4])) { ko = 1; Util.printError("“Key Words：”字体错误，应为Times New Roman"); } } }
                                if (change == 1) { if (ko == 0) { if (!Util.correctRunFonts(run, list[n], doc, "黑体", abstextE[4])) { ko = 1; Util.printError("“Key Words：”字体错误，应为Times New Roman"); } } }
                            ...
                            {
                                // if (change == 0) { if (ky.Replace(" ", "") == "；" && !Util.correctRunFonts(run, list[n], doc, abskeywordE[5], ""))Util.printError("英文摘要关键词分号字体错误，应为宋体"+ky); }
                                if (change == 1) { if (ky.Replace(" ", "") != "；" && ky.Replace(" ", "") != "：" && !Util.correctRunFonts(run, list[n], doc, "黑体", abstextE[8])) { Util.printError("英文摘要关键词的内容字体错误，应为Calibri" + "           " + ky); } }
                                if (change == 0) { if (ky.Replace(" ", "") != "；" && ky.Replace(" ", "") != "：" && !Util.correctRunFonts(run, list[n], doc, "黑体", abstextE[4])) { Util.printError("英文摘要关键词的内容字体错误，应为Times New Roman" + "           " + ky); } }
                                if (ky.Trim() != ky) { rule++; };
                            }
                        }
                    }
