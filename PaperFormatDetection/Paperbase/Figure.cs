...
namespace PaperFormatDetection.Paperbase
{
    class Figure
    {
            ...
            */
            ...
            if (index[0] != -1)
            {
                if (body.ChildElements.GetItem(index[0]).GetType().ToString() == "DocumentFormat.OpenXml.Wordprocessing.Paragraph")
                {
                    ...
                    if (CNtitle != null && isPicInMiddleBefore(Util.pageDic, CNtitle))
                    {
                    ...
                    if (e[0] == 0)
                    {
                        Util.printError("图名序号与“图”之间不应有空格" + "  ----" + s);
                    }
                    if (e[1] == 0)
                    {
                        Util.printError("图名序号与内容之间应有" + MNtoName + "个空格" + "  ----" + s);
                    }
                    if (e[2] == 0)
                    {
                        Util.printError("中文图名序号应为“图M.N”的形式" + "  ----" + s);
                    }
                    else
                    {
                        if (!Util.correctm(num, chapter))
                        {
                            Util.printError("图名序号与章节号不一致，应为图" + chapteR[0] + ".x" + "  ----" + s);
                        }
        ...
        protected void detectCNPicNameUnder(WordprocessingDocument docx, int i)
        {
            Body body = docx.MainDocumentPart.Document.Body;
            int countBodyChild = body.ChildElements.Count();
            Paragraph p = (Paragraph)body.ChildElements.GetItem(i);
            ...
            int[] index = locationOfTitleAndBlankLineUnder(docx, i);
            ...
            if (index[0] != -1)
            {
                if (body.ChildElements.GetItem(index[0]).GetType().ToString() == "DocumentFormat.OpenXml.Wordprocessing.Paragraph")
                {
                    Paragraph pCNtitle = (Paragraph)body.ChildElements.GetItem(index[0]);
                    string CNtitle = Tool.getFullText(pCNtitle).Trim();
                    //Console.WriteLine(CNtitle + index[3]);
                    num = Util.number(CNtitle);//M.N
                    string s = CNtitle;
                    string StrNum = Util.number(s);
                    //中文图名位置
                    ...
                    if (CNtitle != null && isPicInMiddleAfter(Util.pageDic, CNtitle))
                    {
                        if (index[3] == -1)
                        {
                            PaperFormatDetection.Tools.Util.printError("图名之后应空一行，----" + CNtitle);
                        }
                    }
                    #region 图名字体字号居中
                    if (!Util.correctfonts(pCNtitle, docx, "宋体", "Times New Roman"))
                    {
                        PaperFormatDetection.Tools.Util.printError("图名字体错误，应为" + PicCNFont + "  ----" + s);
                    }
                    if (!Util.correctsize(pCNtitle, docx, PicCNFontSize))
                    {
                        PaperFormatDetection.Tools.Util.printError("图名字号错误，应为" + PicCNFontSize + "  ----" + s);
                    }
                    if (!Util.correctJustification(pCNtitle, docx, PicCNFontJustification))
                    {
                        PaperFormatDetection.Tools.Util.printError("图名未" + PicCNFontJustification + "  ----" + s);
                    }
                    #endregion
                    ...
                    if (e[2] == 0)
                    {
                        Util.printError("图名序号应为“图M.N”的形式" + "  ----" + s);
                    }
                    else
                    {
                        if (!Util.correctm(num, chapter))
        ...
        protected void detectENPicName(WordprocessingDocument docx, int i)
        {
            ...
            List<int> iList = Util.getTitlePosition(docx);//标题位置
            string chapter = Util.Chapter(iList, i, body);            //图所在章
            List<int> lists = Tool.getchaptertitleposition(docx);  //章标题
            string chapteR = Util.Chapter(lists, i, body);
            ...
            */
            ...
            if (index[1] != -1)
            {
                if (body.ChildElements.GetItem(index[1]).GetType().ToString() == "DocumentFormat.OpenXml.Wordprocessing.Paragraph")
                {
                    ...
                    if (e[2] == 0)
                    {
                        PaperFormatDetection.Tools.Util.printError("英文图名序号与内容之间应有" + MNtoName + "个空格" + "  ----" + s);
                    }
                    if (e[3] == 0)
                    {
                        PaperFormatDetection.Tools.Util.printError("英文图名序号应为“Fig. M.N”的形式" + "  ----" + s);
                    }
                    else
                    {
                        if (!Util.correctm(num, chapter))
                        {
                            PaperFormatDetection.Tools.Util.printError("图名序号与章节号不一致，应为图" + chapteR[0] + ".x" + "  ----" + s);
                        }
                    ...
                    //英文图名后空行
                    if (ENtitle != null && isPicInMiddleAfter(Util.pageDic, ENtitle))
                    {
                        if (index[3] == -1)
                        {
        ...
        protected bool isPicInMiddleBefore(Dictionary<string, string> pageDic, string picName)
        {
            //通过键的集合取
        ...
        //图(下面）是否在每页的中间 在中间返回true tableName为英文图名
        protected bool isPicInMiddleAfter(Dictionary<string, string> pageDic, string picName)
        {
            //通过键的集合取
            foreach (string keys in pageDic.Keys)
            {
                ...
                {
                    string[] sArray = pageDic[keys].Split('_');// 一定是单引                    
                    int a = int.Parse(sArray[1]);
                    int b = int.Parse(sArray[2]);
                    if (a == b)
                    {
                        return true;
                    }
        ...
        */
        protected int[] locationOfTitleAndBlankLine(WordprocessingDocument wordPro, int picloc)
        {
            ...
            //从pic往前找空格
            int index1 = picloc - 1;
            while (index1 >= 0 && index1 < 1000)//1000防止死循环
            {
                if (body.ChildElements.GetItem(index1).GetType().ToString() == "DocumentFormat.OpenXml.Wordprocessing.Paragraph")
                {
                    Paragraph p = (Paragraph)body.ChildElements.GetItem(index1);
                ...
                else if (body.ChildElements.GetItem(index1).GetType().ToString() == "DocumentFormat.OpenXml.Wordprocessing.BookmarkStart")
                {
                    int count = 0;//找到有几个booksmart
                    do
                    {
                        count++;
                        index1--;
                    } while ((body.ChildElements.GetItem(index1).GetType().ToString() ==
                            "DocumentFormat.OpenXml.Wordprocessing.BookmarkStart" && index1 > 0));
                    if (body.ChildElements.GetItem(index1).GetType().ToString() == "DocumentFormat.OpenXml.Wordprocessing.Paragraph")
                    {
                        Paragraph p = (Paragraph)body.ChildElements.GetItem(index1);
                        string s = p.InnerText.Trim();
            ...
            //从pic往后找 图后空行匹配
            for (int index = picloc + 2; index < picloc + 4 && index < body.Count(); index++)
            {
        ...
        protected int[] locationOfTitleAndBlankLineUnder(WordprocessingDocument wordPro, int picloc)
        {
            int nameindex = 0;
            int[] location = { -1, -1, -1, -1 };
            bool[] find = { false, false, false, false };
            ...
            for (int index = picloc; index <= picloc + 3 && index < body.Count(); index++)
            {
                if (body.ChildElements.GetItem(index).GetType().ToString() == "DocumentFormat.OpenXml.Wordprocessing.Paragraph")
                {
                    Paragraph p = (Paragraph)body.ChildElements.GetItem(index);
            ...
            if (nameindex > 0)
            {
                for (int index = nameindex + 1; index < nameindex + 2 && index + 2 < body.Count(); index++)
                {
                    if (body.ChildElements.GetItem(index).GetType().ToString() == "DocumentFormat.OpenXml.Wordprocessing.Paragraph")
                    {
                        Paragraph p = (Paragraph)body.ChildElements.GetItem(index);
        ...
        protected bool matchPicName(int picloc, Body body)
        {
            ...
            reg[1] = new Regex(@"^图[1-9][0-9]*\.[1-9][0-9]*");//中文图匹配  关键字段：图m.n
            reg[2] = new Regex(@"^图\ *[1-9][0-9]*");//中文图匹配  关键字段：图m
            for (int index = picloc + 1; index < picloc + 2 && index < body.ChildElements.Count(); index++)
            {
                if (body.ChildElements.GetItem(index).GetType().ToString() == "DocumentFormat.OpenXml.Wordprocessing.Paragraph")
                {
                    Paragraph p1 = (Paragraph)body.ChildElements.GetItem(index);
                    string s = p1.InnerText.Trim();
                    if (s.Length > 0 && s.Length < 40)//长度过滤
                    {
                        if (s != "")
                        {
                            //中文图名匹配
        ...
        protected bool itIsPic(Paragraph p)
        {
