...
namespace PaperFormatDetection.Tools
{
    class Util
    {
        ...
        public static string environmentDir = @"C:\Users\lenovo\Desktop\PaperFormatDetection\PaperFormatDetection-master\PaperFormatDetection\PaperFormatDetection\bin\Debug";
        ...
        public static List<Paragraph> sectionLoction(WordprocessingDocument doc, string section, int paperType)
        {
            ...
            foreach (Paragraph p in paras)
            {
                ...
                if (fullText.Replace(" ", "").Equals(section) && p.InnerText.Replace(" ", "").Equals(section))
                {
                    begin = true;
                }
        ...
        {
            string left = "NO", leftchars = "NO", firstline = "NO", firstlinechars = "NO"; 
            //得到这段第一个字符的字号
            firstCharSize = int.Parse(getFirstcharSize(p, doc));
            ////////////////////////
            indentation = (int.Parse(indentation) * 100).ToString();
            string[] array = getFromPpr(p, 0).Split('#');//从段落属性中获取
            left = array[0]; leftchars = array[1]; firstline = array[2]; firstlinechars = array[3];
            if ((left == "NO" || leftchars == "NO" || firstline == "NO" || firstlinechars == "NO") && p.ParagraphProperties != null)//段落属性中没找到
            {
                ParagraphStyleId style_id = p.ParagraphProperties.ParagraphStyleId;
                if (style_id != null)//从style中获取
                {
                    array = getFromStyle(doc, style_id.Val, 0).Split('#');
                    if (left == "NO") left = array[0];
                    if (leftchars == "NO") leftchars = array[1];
                    if (firstline == "NO") firstline = array[2];
                    if (firstlinechars == "NO") firstlinechars = array[3];
                }
            }
            if (left == "NO" || leftchars == "NO" || firstline == "NO" || firstlinechars == "NO")//style没找到
            {
                array = getFromDefault(doc, 0).Split('#');
                if (left == "NO") left = array[0];
                if (leftchars == "NO") leftchars = array[1];
                if (firstline == "NO") firstline = array[2];
                if (firstlinechars == "NO") firstlinechars = array[3];
            ...
            double ind = 0;
            if (leftchars != "NO" && leftchars!="0")
               ind += double.Parse(leftchars);
            else if (left != "NO")
                ind += double.Parse(PointToChars(left));
            if (firstlinechars != "NO" && firstlinechars!="0")
                ind += double.Parse(firstlinechars);
            else if (firstline != "NO")
                ind += double.Parse(PointToChars(firstline));
            //Console.WriteLine(p.InnerText);
            //Console.WriteLine("left: " + left + " leftchars: " + leftchars + " firstline: " + firstline + " firstlinechars: " + firstlinechars);
            //Console.WriteLine(ind + "   " + getTabInd(p, doc) / 2.4 + "    " + double.Parse(indentation));
            if (Math.Abs(ind + getTabInd(p, doc)/2.4 - double.Parse(indentation)) <= 10)
                return true;
            else
                return false;
        }
        public static double getTabInd(Paragraph p, WordprocessingDocument doc)
        {           
            //目录
            ...
            {
                if (r.GetFirstChild<FieldChar>() != null)
                {
                    flag = true;
                }
            }
            ...
            {
                if (r.GetFirstChild<TabChar>() != null)
                {
                    hasTab = true;
                    string tab = getFromPpr(p, 9);
                    if (tab == null && p.ParagraphProperties != null)//段落属性中没找到
                    {
                        ParagraphStyleId style_id = p.ParagraphProperties.ParagraphStyleId;
                        if (style_id != null)//从style中获取
                        {
                            tab = getFromStyle(doc, style_id.Val, 9);
                        }
                    }
                    if (tab == null)
                        tab = getFromDefault(doc, 9);
                    if (tab == null)
                        tab = "0";
                    if (tab != null)
                        tabsNum += double.Parse(tab);
                }
                else
                    break;
            }
            if (hasTab){
                //Console.WriteLine("66666   "+tabsNum);
                return tabsNum;
            }
            else
                return 0.0;
        ...
        public static bool correctSpacingBetweenLines_Be(Paragraph p, WordprocessingDocument doc, string before)
        {
            ...
            if (int.Parse(be) % 156 == 0)
                be = ((Double.Parse(be) / 312.0) * 240).ToString();
            if (int.Parse(be) % 163 == 0)
                be = ((Double.Parse(be) / 326.0) * 240).ToString();

            /*if (getSize(p, doc) == "30" && Double.Parse(be) / 312.0 == Double.Parse(before) / 240.0)
                return true;
            if (getSize(p, doc) == "32" && Double.Parse(be) / 326.0 == Double.Parse(before) / 240.0)
                return true;*/
        ...
        public static bool correctSpacingBetweenLines_Af(Paragraph p, WordprocessingDocument doc, string after)
        {
            ...
            if (int.Parse(af) % 156 == 0)
                af = ((Double.Parse(af) / 312.0) * 240).ToString();
            if (int.Parse(af) % 163 == 0)
                af = ((Double.Parse(af) / 326.0) * 240).ToString();

            /*if (getSize(p, doc) == "30" && Double.Parse(af)/312.0==Double.Parse(after) / 240.0)
                return true;
            if (getSize(p, doc) == "32" && Double.Parse(af) / 326.0 == Double.Parse(after) / 240.0)
                return true;*/
        ...
        public static bool correctsize(Paragraph p, WordprocessingDocument doc, string size)
        {
            ...
            foreach (Run run in runlist)
            {
                if (Regex.IsMatch(run.InnerText, @"^([ ])+$") || (run.GetFirstChild<Text>() == null))
                    continue;
        ...
        //获取字体大小
        public static string getSize(Paragraph p, WordprocessingDocument doc)
        {
            string each = null;
            Run run=p.GetFirstChild<Run>();
            if (run != null && run.GetFirstChild<Text>()!=null && run.InnerText.Replace(" ","")!="")
            {
                each = getFromRunPpr(run, 6); //从Run属性中查找
                if (each == null)
                {
        ...
        public static bool correctRunFonts(Run run, Paragraph p,WordprocessingDocument doc, string CNfont,string ENfont)
        {
            ...
            {
                if ((type == 7 && font == ENfont) || (type == 8 && font == CNfont))
                    return true;
                else
                    return false;
            }
            ...
            if (ppr != null)
            {
                ...
                if (type == 0)
                {
                    ...
                    if (ind != null)
                    {
                        ...
                        else if (ind.FirstLineChars != null && ind.FirstLine == null && ind.FirstLineChars == "0")
                            return null;
                    ...
                    return left + "#" + leftchars + "#" + firstline + "#" + firstlinechars;
                }
                /////对齐方式
                if (type == 1)
                {
                ...
                if (type == 9)
                {
                    ...
                    if (tab != null)
                    {
                        IEnumerable<TabStop> tabstops = tab.Elements<TabStop>();
        ...
        {
            if (double.Parse(px) % 120 == 0)
            {
                string l = (double.Parse(px) / 240).ToString();
                return l+"行";
            }
            else {
                return (double.Parse(px) / 20).ToString() + "磅";
            }
        }
